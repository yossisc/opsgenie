...
################################
#    CHANGE HERE !!!
start_date = datetime(2026, 8, 1)   # yyyy, mo, day
end_date   = datetime(2026, 8, 31)  # yyyy, mo, day
#
################################

first_workday_user = "Yossi"
first_weekend_user = "Gabi"
workday_rotation = ["Yossi", "Nadav", "Gabi", "Dovid"]
...
WEEKLY_PAIRS = [
    ...
    ["Nadav", "Dovid"],   # Week 4 (swap of week 2)
]
...
all_team_members = ["Yossi", "Gabi", "Nadav", "Dovid"]
...
########################################################################################
        ...
        # ── REGULAR WORKDAY handler (Sun–Thu) — NEW date-anchored rotation ──
            ...
            # Full-day shift for everyone (no separate night coverage)
...
if apply_run:
    ...
    for entry in non_overlapping_schedule:
        ...
        date_from_dt = datetime.strptime(date_from, "%d/%m/%Y")
        if date_from_dt >= start_date and date_from_dt <= end_date:
            if member in ('Extra1', 'Extra2', 'Extra3'):
                member = 'Gabi'
