...
parser.add_argument('--first_workday', type=str)
parser.add_argument('--first_weekend', type=str)
...
end_date = datetime(2024, 3, 1)  # yyyy, mo, day
#start_date = datetime(2024, 4, 8 ) # yyyy, mo, day
#end_date = datetime(2024, 5, 31)  # yyyy, mo, day
#
################################

first_workday_user = args.first_workday
first_weekend_user = args.first_weekend
...
def rotate_and_get_next_person(people_list):
    before_rotate = people_list[0]
    people_list.append(people_list.pop(0))
    return before_rotate
...
#            if day_name != "Friday":
#                print(f"{day_name} - {weekend_rotation[0]} - {assigned_member}")
            assigned_member = rotate_and_get_next_person(weekend_rotation)
...
##                schedule[-2] = tuple(second_last_item)
            ...
            print(f"weekend_rotation - {weekend_rotation}")
            print(f"weekend_rotation_when_dati- {weekend_rotation_when_dati}")
            if assigned_member == "Moriah" or assigned_member == "Dovid":
                ...
                goy_shel_shabat = rotate_and_get_next_person(weekend_rotation)
...
####                schedule.append((current_date.strftime("%d/%m/%Y"), (current_date + timedelta(days=1)).strftime("%d/%m/%Y"), format_time("2:00PM"), format_time("8:00PM"), next_assigned_member,"WEEKEND"))
                #
                # Sat 8pm - Sun 8am
                ...
                # while(marker_member != rotate_and_get_next_person(weekend_rotation)):
                # print(f"{assigned_member} {weekend_rotation[0]} ")
                # pass
...
# NEXT....
print("| Date From  | Date To    | Time From | Time To    | Team Member | Weekend    |")
print("|------------|------------|-----------|------------|-------------|------------|")
for entry in non_overlapping_schedule:
    ...
    if datetime.strptime(date_from, "%d/%m/%Y") >= start_date and datetime.strptime(date_to, "%d/%m/%Y") <= end_date:
        ...
        if we_or_reg == "WEEKEND":
            ...
            print("\033[1;91m", end="")
        print(f" {date_from_formatted:<12} {date_to_formatted:<12} {time_from:<12} {time_to:<12} {member:<12} {we_or_reg:<12}")
